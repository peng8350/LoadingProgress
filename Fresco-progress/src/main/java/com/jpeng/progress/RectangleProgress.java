...
public class RectangleProgress extends BaseProgress {
	...
	private void drawProgress(Canvas canvas, Rect rect) {
		...
		canvas.drawRect(rect.left + rect.width() / 10, rect.centerY() - mRecHeight,
				(float) ((rect.width() - rect.width() / 5) * (double) ((double) mProgress / (double) MAX_VALUE))
						+ rect.width() / 10,
	...
	public void DrawText(Canvas canvas) {
		...
		if (mBitmap == null) {
			...
			int baseline = size.top + (size.bottom - size.top - fontMetrics.bottom + fontMetrics.top) / 2
					- fontMetrics.top;
			canvas.drawText(mProgress / 100 + "%",
					(float) ((size.width() - size.width() / 5) * (double) ((double) mProgress / (double) MAX_VALUE))
							+ size.width() / 10 + 15,
